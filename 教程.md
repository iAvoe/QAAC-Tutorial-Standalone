...
和封装/音频不同，将文件作为真的"流格式"编码，一般用于直播和蓝牙A2DP音频。
...
Windows 10 2004或更高版本可通过[Github上的第三方方案](https://github.com/ysc3839/AudioPlaybackConnector/tree/master)实现蓝牙ADTS输入，将系统本身临时作为接收设备用，但原作者长期未更新，且仅支持手动连接，有需求的可以fork一个~
...
**采样率**：
 ...
 - 一首歌的采样率是各个声道的采样率加起来的和，比如歌曲44100Hz = 左声道L-Ch 22050Hz + 右声道R-Ch 22050Hz
   - 这个计数方法很离谱，就比如窗口1播放60fps的视频，窗口2播放60fps的视频，等于120fps的视频一样...
...
综上所述，只要确认要编码音频的音高赫兹数，就可以确定使用更低的采样率编码音频，实现压缩

    最高有效音高赫兹数     × 音轨通道数量        = 有效音频采样率
    Max Eff. Pitch Hertz × Number of Channels = Effective Audio Sample Rate
